...
# id -> (rail label, amenity name in plots-3d.json, clip title or None, copy)
STOPS = [
    ...
    ("plaza", "Central Green", "Lawn & Plaza", None,
     "Lawn, shaded walks and seating pockets running the length of the clubhouse frontage."),
...
def main() -> int:
    ...
    ap.add_argument("--scroll-per-unit", type=float, default=0.75)
    args = ap.parse_args()
    ...
    gate_station = {
        "id": "gate", "label": "Entrance", "title": "The Arrival",
        "description": "You turn in off the highway. The boulevard opens ahead "
                       "and the township unfolds from here.",
        "at": [round(GATE[0], 2), round(GATE[1], 2)],
    ...
    config = {
        "_readme": [
            ...
            "Stations with fullscreenVideo take the whole screen on arrival and",
            "scrub the amenity clip with scroll, then hand back to the 3D drive.",
        ],
